...
# creat a {pid:arrival_time} dict and then we can sort them by their arrival time 
	...
	for pid in pid_dict:
		...
		print('pid:%6d   arrival time:%6d   burst time:%6d   wait time:%6d   timer:%6d'\
			%(pid[0],pid[1], node.burst_time,waiting_time,timer))
	...
	while len(data)>0:
		...
		for i in range(len(data)):
			...
			if data.loc[i]['arrival_time']>timer or i==len(data)-1 :
				...
				print('pid:%6d   arrival time:%6d   burst time:%6d   wait time:%6d   priority:%6d   time:%6d'\
					%(data.loc[flag]['pid'],data.loc[flag]['arrival_time'], data.loc[flag]['burst_time'],waiting_time,data.loc[flag]['priority'],timer))
...
def round_robin(queue,q):
	...
	print('\n-------------------------------------------------------------------------------------------------------',end='')
	print('\nUsing Round Robin Priority Scheduling, execution sequence is:\n')
	while len(data)>0:
		...
		for i in range(len(data)):
			...
			if pcb.remaining_time>q:
				...
				print('Execute pid:%6d   arrival time:%6d   burst time:%6d   remaining time:%6d   time:%6d'\
